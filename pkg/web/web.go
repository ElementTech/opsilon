...
func App(port int64, v string) {
	...
	rgw.GET("/list/:workflow", wlist).
		AddResponse(http.StatusOK, "list workflows that have been run by this server", nil, nil).
		AddParamPath("", "workflow", "workflow id to view (generated by hashing the workflow), omit to view all")

	rrgw := e.Group("run", "/api/v1/run")
	rrgw.GET("/list/:workflow", wrlist).
...
func wlist(c echo.Context) error {
	...
	docs := []internaltypes.Workflow{}
	err := db.FindManyWorkflows("workflows", filter, docs)
...
func wrlist(c echo.Context) error {
	workflow := c.Param("workflow")
	docs := []internaltypes.Result{}
	err := db.FindManyResults("results", bson.D{{Key: "workflow", Value: workflow}}, docs)
